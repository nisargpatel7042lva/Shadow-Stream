...
# Fix is_sorted_by - comment out or replace with manual implementation
# Note: These features require Rust 1.82+, so we'll comment them out for now
find "$INDEXMAP_DIR/src" -name "*.rs" -exec sed -i 's/\.is_sorted_by(/\/\/ .is_sorted_by(/g' {} \; 2>/dev/null
find "$INDEXMAP_DIR/src" -name "*.rs" -exec sed -i 's/\.is_sorted_by_key(/\/\/ .is_sorted_by_key(/g' {} \; 2>/dev/null
